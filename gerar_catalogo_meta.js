...
async function start() {
    console.log("🚀 Iniciando Motor de Sincronização do Catálogo Meta (Facebook/WhatsApp)");
    ...
    const csvPath = path.join(rootDir, 'catalogo_meta.csv');
    // Adiciona o BOM UTF-8 (\uFEFF) para o Facebook ler os emojis perfeitamente!
    fs.writeFileSync(csvPath, '\uFEFF' + csvContent, 'utf-8');
    
    console.log(`\n🎉 SUCESSO! ${processedCount} produtos foram convertidos.`);
    console.log(`📁 O arquivo [catalogo_meta.csv] foi criado com sucesso na pasta raiz!`);
}
